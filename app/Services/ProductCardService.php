...
class ProductCardService
{
    public function format($products): array
    {
        return collect($products)->map(fn($product) => [
            ...
            'price'         => $product->price,
            'discountPrice' => ($product->discount && $product->discount_price) ? (float) $product->discount_price : null,
            'badge'         => ($product->discount && $product->discount_price) ? 'Sale' : ($product->is_new_arrival ? 'New' : null),
