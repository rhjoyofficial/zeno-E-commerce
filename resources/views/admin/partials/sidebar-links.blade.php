...
<div>
    <button data-accordion-btn="products-menu"
        ...
        <svg id="products-menu-chevron"
            class="ml-2 h-4 w-4 text-gray-400 transition-transform duration-200 flex-shrink-0 {{ request()->routeIs('admin.products.*') ? 'rotate-90' : '' }}"
            fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
        </svg>
    </button>

    <div id="products-menu" class="ml-5 pl-2 mt-1 space-y-1 {{ request()->routeIs('admin.products.*') ? '' : 'hidden' }}">
...
<a href="{{ route('admin.coupons.index') }}"
    class="group flex items-center px-2 py-2 text-sm font-medium rounded-lg {{ request()->routeIs('admin.coupons.*') ? 'bg-indigo-100 text-indigo-700' : 'text-gray-600 hover:text-indigo-600 hover:bg-indigo-50' }}">
    <svg class="mr-3 h-5 w-5 flex-shrink-0 {{ request()->routeIs('admin.coupons.*') ? 'text-indigo-500' : 'text-gray-400 group-hover:text-indigo-500' }}"
        xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A2 2 0 013 12V7a4 4 0 014-4z" />
    </svg>
...
<!-- Home Sections -->
<div>
    <button data-accordion-btn="home-sections-menu"
        ...
        <svg id="home-sections-menu-chevron"
            class="ml-2 h-4 w-4 text-gray-400 transition-transform duration-200 flex-shrink-0 {{ request()->routeIs('admin.home-sections.*') ? 'rotate-90' : '' }}"
            fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
        </svg>
    </button>

    <div id="home-sections-menu" class="ml-5 pl-2 mt-1 space-y-1 {{ request()->routeIs('admin.home-sections.*') ? '' : 'hidden' }}">
...
(function () {
    document.querySelectorAll('[data-accordion-btn]').forEach(btn => {
        const panelId = btn.dataset.accordionBtn;
